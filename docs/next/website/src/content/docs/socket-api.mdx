...
Current official integrations may include an allow-listed `runtime` object in either report method. Supported fields are `model`, `model_provider`, `reasoning_effort`, `reasoning_summary`, `verbosity`, `service_tier`, `context_tier`, `variant`, and `agent_profile`. Herdr validates fields against the matching official `source` and `agent`; arbitrary command arguments and provider configuration are not accepted.
...
If no native session reference is stored, `agent_session` is omitted. If Herdr has not observed a runtime profile for that session, `runtime` is omitted. Codex and GitHub Copilot CLI runtime responses are also refreshed from their provider-owned session event logs; Claude Code and OpenCode refresh through their managed integrations.
