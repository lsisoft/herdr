...
For Codex, Claude Code, GitHub Copilot CLI, and OpenCode, Herdr also records an allow-listed access and runtime profile. Permission switches such as `--yolo` and the provider-supported model/compute options are reapplied to the native resume command. Codex and Copilot settings are read from their provider-owned session event logs, Claude reports model and effective effort through its hook, and OpenCode reports provider/model, variant, and agent through its plugin.

Native session restore requires current Herdr integrations: Pi integration version `4`, OMP version `4`, Claude Code version `8`, Codex version `6`, GitHub Copilot CLI version `2`, Devin CLI version `2`, Droid version `2`, Kimi Code CLI version `4`, Qoder CLI version `2`, Cursor Agent CLI version `1`, OpenCode version `9`, Kilo Code CLI version `2`, Hermes Agent version `3`, or MastraCode version `1`. Check installed versions with `herdr integration status`.
