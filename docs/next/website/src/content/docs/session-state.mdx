...
For Codex, Claude Code, GitHub Copilot CLI, and OpenCode, the snapshot also stores an allow-listed launch profile. Herdr preserves documented permission flags plus the latest model and compute settings it can observe from process arguments, provider-owned session events, or the managed integration. On restore, those provider-supported options are added to the native resume command. Herdr does not persist prompts, credentials, environment variables, or arbitrary provider configuration as part of this profile.

The restorable runtime fields differ by provider:

| Agent | Recorded and restored runtime settings |
| --- | --- |
| Codex | Model, model provider, reasoning effort/summary, verbosity, and service tier |
| Claude Code | Model, effort, and agent profile |
| GitHub Copilot CLI | Model, reasoning effort, context tier, custom agent, and enabled reasoning summaries; provider event metadata such as verbosity is recorded even when the CLI has no matching resume option |
| OpenCode | Provider/model and agent profile are restored; the observed variant is recorded, but OpenCode does not document a variant flag for TUI session resume |
...
| --- | --- | --- |
| Pi | `4` | `pi --session <path-or-id>` |
| OMP | `4` | `omp --resume=<path-or-id>` |
| Claude Code | `8` | `claude --resume <id>` |
| Codex | `6` | `codex resume <id>` |
...
| Kimi Code CLI | `4` | `kimi --session <id>` |
| Qoder CLI | `2` | `qodercli --resume <id>` |
| OpenCode | `9` | `opencode --session <id>` |
| Kilo Code CLI | `2` | `kilo --session <id>` |
| Hermes Agent | `3` | `hermes --resume <id>` |
