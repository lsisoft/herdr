...
Session references and any observed allow-listed permission/model profile are stored in the local Herdr session snapshot. For Codex, Claude Code, GitHub Copilot CLI, and OpenCode, Herdr reapplies the documented provider options it can safely restore. `pane get`, `pane list`, `agent get`, and `agent list` expose the model profile under `agent_session.runtime`; no prompts, credentials, or arbitrary provider configuration are copied into the snapshot.
